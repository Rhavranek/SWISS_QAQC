...
DryAirResults_070521 <- DryAirResults_070521  %>% 
  bind_rows(data_070521 %>%
    filter(MDT > "2021-07-05 17:01:55" & MDT < "2021-07-05 16:04:55") %>% 
      flask_out_ave() %>% 
        mutate(Flask = 16)
  )
...
outlet_smart_aves <- outlet.cut %>% 
 ...
 # ungroup %>% 
  #select(UTC, batch, -row, everything())
...
next step: 
- take each batch and throw away the first 90 seconds
- then, average shit for the following 180 seconds

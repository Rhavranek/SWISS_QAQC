...
I also did flask 5 manually becuase it was the only flask with a tail 
...
WVtest_results <- flasks_031922 %>% 
  left_join(flasks_040222, by = "Flask") %>% 
    mutate(
      ...
      d18O_corr_diff = d18O_corr.y -d18O_corr.x,
      d2H_diff_raw =   d2H_out.y - d2H_out.x,
      d2H_corr_diff = d2H_corr.y - d2H_corr.x,
      phlox_diff = d18O_corr_phlox - d18O_corr.x,
      d2h_phlox = d2H_corr_phlox -d2H_corr.x
    )


WVtest_results_plt <- WVtest_results %>%  
  filter(Flask < 15) %>% 
  pivot_longer(cols = c(d18O_corr_diff, d2H_corr_diff)) %>% 
    mutate(
      # turn into ordered variable (factor) that keeps the order
      ...
      latex_name = as_factor(name) %>% fct_inorder()  %>%
        # introduce latex substitutions
        fct_recode(
                   "$\\Delta \\delta^2H" = "d2H_corr_diff",
                   "$\\Delta \\delta^{18}O" = "d18O_corr_diff")
    ) %>%
      ggplot()+
...
WVtest_results_plt_202208 <- WVtest_results_202208 %>%  
  pivot_longer(cols = c(d18O_diff_raw, d2H_diff_raw)) %>% 
    mutate(
      # turn into ordered variable (factor) that keeps the order
      ...
      latex_name = as_factor(name) %>% fct_inorder()  %>%
        # introduce latex substitutions
        fct_recode(
                   "$\\Delta \\delta^2H" = "d2H_diff_raw",
                   "$\\Delta \\delta^{18}O" = "d18O_diff_raw")
    ) %>%
      ggplot()+
        aes(x = Flask, y = value, size = 2) +
        facet_wrap(~latex_name, labeller = latex_labeller, ncol = 1, scales = "free")+

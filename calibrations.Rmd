...
# Briggsdale Correction
```{r}
all_0812 <- bind_rows(stds_081022, stds_081122, stds_081222) %>% 
  liq_rothfuss2013() %>% 
  left_join(stds, by = "std")

soconfunsed <- all_0812 %>% 
  ggplot()+
  aes(x = d18o_known, y = d18O_liq)+
  geom_point() +
  geom_abline(aes(slope = 1.005, intercept = 0.3055))

ggplotly(soconfunsed)

d18O_lm_0812 <- lm(d18O_liq ~ d18o_known, all_0812) 
dD_lm_0812 <- lm(d2H_liq ~ d2H_known, all_0812)
...
d18O_corr = (d18O_out - -8.994) / 1.000
d2H_corr = (d2H_out - -80.7073) / 0.92

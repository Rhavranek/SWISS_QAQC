...
July 2, 2023 - Changed the time zone on the cut data to match notes. If running this in Colorado, re-update times.  
```{r}
data_080522 <- tbl %>% 
...
```{r}
all_0822 <- bind_rows(stds_080922, stds_081022, stds_081122, stds_081222) %>% 
  liq_rothfuss2013() %>% 
  left_join(stds, by = "std")
...
d18O_corr = (d18O_liq - 0.3276101) / 1.0067558
d2H_corr = (d2H_out - -7.408472) / 1.009117
